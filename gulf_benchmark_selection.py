...
# Map single-letter industry codes to full industry names
INDUSTRY_CODE_MAP = {
    "a": "Aluminum",
    "c": "Chemicals",
    "ce": "Cement",
...
def map_to_standard_industry(industries):
    ...
    for industry in industries:
        ...
        elif industry.lower() in INDUSTRY_CODE_MAP.values():
            mapped.add(industry)
        else:
            # Try to match based on common patterns
            if "oil" in industry.lower() or "gas" in industry.lower():
                mapped.add("Oil & Gas")
            elif "chem" in industry.lower():
                mapped.add("Chemicals")
            elif "food" in industry.lower() or "beverage" in industry.lower():
                mapped.add("Food & Beverage")
            elif "const" in industry.lower():
                mapped.add("Construction")
            elif "med" in industry.lower():
                mapped.add("Medical Devices")
            elif "agri" in industry.lower() or "fert" in industry.lower():
                mapped.add("Agriculture/Fertilizers")
            elif "water" in industry.lower():
                mapped.add("Water Desalination")
            elif "alum" in industry.lower():
                mapped.add("Aluminum")
            elif "steel" in industry.lower():
                mapped.add("Steel")
            elif "cement" in industry.lower():
                mapped.add("Cement")
            elif "glass" in industry.lower():
                mapped.add("Glass")
            elif "plastic" in industry.lower() or "paper" in industry.lower():
                mapped.add("Paper & Plastic")
            elif "electron" in industry.lower():
                mapped.add("Electronics")
            elif "gases" in industry.lower():
...
def map_to_standard_industry(industries):
    """Map granular industries to standard sectors with improved matching"""
    mapped = set()
    for industry in industries:
        # Normalize industry name
        normalized = industry.lower().replace('_', ' ').replace('-', ' ')
        
        # Check for matches in all required sectors
        if any(word in normalized for word in ["oil", "gas", "petro", "lng"]):
            mapped.add("Oil & Gas")
        if any(word in normalized for word in ["aluminum", "aluminium"]):
            mapped.add("Aluminum")
        if "steel" in normalized:
            mapped.add("Steel")
        if "cement" in normalized:
            mapped.add("Cement")
        if any(word in normalized for word in ["food", "beverage", "dairy", "poultry", "milk", "juice"]):
            mapped.add("Food & Beverage")
        if any(word in normalized for word in ["chemical", "polymer", "petrochemical"]):
            mapped.add("Chemicals")
        if "glass" in normalized:
            mapped.add("Glass")
        if any(word in normalized for word in ["paper", "plastic", "packaging"]):
            mapped.add("Paper & Plastic")
        if any(word in normalized for word in ["electronic", "semiconductor", "chip"]):
            mapped.add("Electronics")
        if any(word in normalized for word in ["construction", "building", "contracting"]):
            mapped.add("Construction")
        if any(word in normalized for word in ["water", "desalination", "desalinization"]):
            mapped.add("Water Desalination")
        if any(word in normalized for word in ["medical", "healthcare", "device"]):
            mapped.add("Medical Devices")
        if any(word in normalized for word in ["agriculture", "fertilizer", "crop", "farm"]):
            mapped.add("Agriculture/Fertilizers")
        if any(word in normalized for word in ["gas", "gases", "nitrogen", "oxygen"]):
            mapped.add("Industrial Gases")
        if any(word in normalized for word in ["mining", "mineral", "ore"]):
            mapped.add("Mining")
    
    # If we found matches, return them. Otherwise, return the original industry.
    if mapped:
        return list(mapped)
    else:
        # Return the original industry as a list
        return industries
...
for company in companies:
    ...
    company['standard_industries'] = map_to_standard_industry(company['industry'])
